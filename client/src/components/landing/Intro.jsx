...
import { PlaceCardSkeleton2 } from "../UI/LoaderSkeletons";
...
const Intro = () => {
  ...
  async function fetchPlaceSummary() {
    try {
      ...
      const res = await fetch(`${BASE_URL}/places/summaries/s1`);
      const result = await res.json();

      if (res.status === 200) setPlaces(result.data);
    } catch (err) {
  ...
  return (
    <section
      id="intro"
      className="w-full min-h-screen relative pb-8 overflow-hidden bg-gray-800"
    >
      <div
